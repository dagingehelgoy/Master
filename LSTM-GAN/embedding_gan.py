...
from data_gen import generate_input_noise, generate_embedding_captions
from enums import Conf
from list_helpers import pairwise_cosine_similarity
...
def s_discriminator_model(config):
	model = Sequential()
	model.add(LSTM(
		50,
...
def emb_predict(config, logger):
	...
	word_list_sentences, word_embedding_dict = generate_embedding_captions(config)
