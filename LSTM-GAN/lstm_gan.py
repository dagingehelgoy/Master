...
from data_gen import generate_index_captions, generate_input_noise, to_categorical_lists, onehot_to_softmax, \
	generate_embedding_captions, emb_get_training_batch
from embedding_gan import emb_create_generator, emb_create_discriminator
from enums import WordRepr, Conf
from onehot_gan import oh_create_generator, oh_create_discriminator
...
def train(gan_logger, config):
	...
	if config[Conf.WORD_REPR] == WordRepr.ONE_HOT:
		...
		training_data, word_embedding_dict = generate_embedding_captions(config)
	...
	for epoch_counter in range(config[Conf.EPOCHS]):
		print("Epoch: %s" % epoch_counter)
		np.random.shuffle(training_data)
		for batch_counter in range(nb_batches):
			noise_batch = generate_input_noise(config)
			training_batch = training_data[batch_counter * config[Conf.BATCH_SIZE]:(batch_counter + 1) * config[Conf.BATCH_SIZE]]
			...
			# for _ in range(1):
				...
				# 	print "#" * 50
				# 	gan_logger.save_model_weights(g_model, epoch_counter, batch_counter, "generator",
				# 	                              suffix="LOCAL-MINIMA")
				# 	gan_logger.save_model_weights(d_model, epoch_counter, batch_counter, "discriminator",
				# 	                              suffix="LOCAL-MINIMA")
				# 	return
			...
			if batch_counter % int(nb_batches / 1) == 0:
				...
				gan_logger.save_loss(g_loss, d_loss_gen, epoch_counter, batch_counter)

		if (epoch_counter < 100 and epoch_counter % 10 == 0) or epoch_counter % 500 == 0:
			gan_logger.save_model_weights(g_model, epoch_counter, "generator")
			gan_logger.save_model_weights(d_model, epoch_counter, "discriminator")
